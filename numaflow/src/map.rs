...
impl SystemMetadata {
    ...
    ///
    /// ```no_run
    /// use numaflow::map::SystemMetadata;
    /// let mut smd = SystemMetadata::default();
    /// smd.add_kv_string("group1", "key1", "value1");
    /// smd.add_kv_string("group2", "key2", "value2");
    ...
    /// smd.add_kv_string("group1", "key1", "value1");
    /// smd.add_kv_string("group2", "key2", "value2");
    /// let value = smd.value("group1", "key1");
    /// println!("{:?}", value);
    /// ```
...
impl UserMetadata {
    ...
    /// ```
    pub fn value(&self, group: &str, key: &str) -> Vec<u8> {
        self.data.get(group).and_then(|kv| kv.get(key)).cloned().unwrap_or_default()
    }
